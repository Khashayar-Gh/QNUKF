...
% Read the dataset
[IMURead, ViconRead, Groundtruth, CamRead]= dataRead(path , setpath); 
...
Tfinal     = ceil(length(TimerIMU)*0.98);%20000;%
...
% P0_ = blkdiag(zeros(3)+eye(3)*10^3*5 , zeros(3)+eye(3)*6*10^2 , 1*10^1*eye(3) , eye(6)*10^1); % use for V2
% P0_ = blkdiag(zeros(3)+eye(3)*10^2*5 , zeros(3)+eye(3)*6*10^2 , 1*10^1*eye(3) , eye(6)*10^0); % use for V1
% P0_ = blkdiag(zeros(3)+eye(3)*10^0*1 , zeros(3)+eye(3)*1*10^0 , 1*10^0*eye(3) , eye(6)*10^0); % for case 5
% P0_ = blkdiag(zeros(3)+eye(3)*10^0*10 , zeros(3)+eye(3)*2*10^0 , 11*10^0*eye(3) , eye(6)*10^0);
% P0_ = blkdiag(zeros(3)+eye(3)*10^0*4 , zeros(3)+eye(3)*6*10^2 , 1*10^1*eye(3) , eye(6)*10^1*0); %data_QNUKF_20240810_214134_1.mat EKF case 7
% P0_ = blkdiag(zeros(3)+eye(3)*10^2*5 , zeros(3)+eye(3)*6*10^2 , 1*10^1*eye(3) , eye(6)*10^0*0); %QNUKF compare
...
omega_mean = mean(IMURead(15*200:end,2:4)) ;
a_mean     = mean(IMURead(15*200:end,5:7)) ;
...
filename = ['data_QNUKF_' dateTimeString '_' num2str(setpath) '.mat'];
...
function [IMURead,ViconRead,Groundtruth,CamRead] = dataRead(path , path_num)
