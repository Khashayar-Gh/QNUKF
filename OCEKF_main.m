...
switch  setpath
    ...
    case 3
        path        = 'C:\Offline\Datasets\V1_03_difficult\mav0'; 
    case 4
        path        = 'C:\Offline\Datasets\V2_01_easy\mav0'; 
        
    case 5
        path        = 'C:\Offline\Datasets\V2_02_medium\mav0';
        %didn't work
    case 6
        path        = 'C:\Offline\Datasets\V2_03_difficult\mav0';
        %didn't work
end
...
Tfinal     = ceil(length(TimerIMU)*0.20);%20000;%
...
% qhat = 1.5*qhat+3*[2 , 3 , -2 , 1]'; %case 3
% qhat = 1.5*qhat+0.3*[2 , 3 , -2 , 1]'; %case 5
% qhat = 1.0*qhat+0.03*[2 , 3 , -2 , 1]'; %case 6
qhat = 1.0*qhat+8.00*[2 , 3 , -2 , 1]'; %ukf
% qhat = 1*qhat+[2 , 3 , -2 , 1]'*0.2; %%data_QNUKF_20240810_214134_1.mat EKF
qhat = qhat/norm(qhat);
% n = [n_w , n_wb , n_a  , n_ab] OCEKF
% x = [q , b_w , v  , b_a, p] OCEKF
% Xhat = [qhat ; 1.00*p+7.0*[1, 1, -2]'*0.1 ;...
%     0*v+0.0*randn(3,1);bw'*1;ba'*1]; %%data_QNUKF_20240810_214134_1.mat EKF
% Xhat = [qhat 
%     bw'*1
%     0*v+0.0*randn(3,1)
%     ba'*1
%     1.00*p+7.0*[1, 1, -2]'*0.1]; %%OCEKF data_OCEKF_20240815_204319_1.mat
% Xhat = [qhat 
%     bw'*1
%     1*v+0.0*randn(3,1)
%     ba'*1
%     1.00*p+7.0*[1, 1, -2]']; %%OCEKF Mid
Xhat = [qhat 
    bw'*1
    1*v+0.0*randn(3,1)
    ba'*1
    1.50*p+1.5*[1, 1, -2]']; %%ukf
...
P_q_indices = {[1 , 2 , 3]};
dim_n = 12;
[state, params] = initialize_ocekf(dim_x, dim_z, dt, dim_n,...
    q_indices, P_q_indices);
...
% P0_ = QUKF2OCEKFcov(blkdiag(zeros(3)+eye(3)*10^3*5 , zeros(3)+eye(3)*6*10^2 , 1*10^1*eye(3) , eye(6)*10^1)); % use for V2
% P0_ = blkdiag(zeros(3)+eye(3)*10^3*5 , zeros(3)+eye(3)*6*10^2 , 1*10^1*eye(3) , eye(6)*10^1); % use for V1
% P0_ = blkdiag(zeros(3)+eye(3)*10^0*1 , zeros(3)+eye(3)*1*10^0 , 1*10^0*eye(3) , eye(6)*10^0); % for case 5
% P0_ = blkdiag(zeros(3)+eye(3)*10^0*4 , zeros(3)+eye(3)*6*10^2 , 1*10^1*eye(3) , eye(6)*10^1*0); %data_QNUKF_20240810_214134_1.mat EKF
% n = [n_w , n_wb , n_a  , n_ab] OCEKF
% x = [q , b_w , v  , b_a, p] OCEKF
% P0_ = blkdiag(zeros(3)+eye(3)*10^0*4 ,...
%     eye(3)*10^1*0 ,...
%     1*10^1*eye(3),...
%     eye(3)*10^1*0 ,...
%     zeros(3)+eye(3)*6*10^2); %data_QNUKF_20240810_214134_1.mat EKF - OC eq
% P0_ = blkdiag(zeros(3)+eye(3)*10^2*5 ,...
%     eye(3)*10^1*0 ,...
%     1*10^1*eye(3),...
%     eye(3)*10^1*0 ,...
%     zeros(3)+eye(3)*6*10^2); %New OC
% P0_ = blkdiag(zeros(3)+eye(3)*10^2*9 ,...
%     eye(3)*10^1*0 ,...
%     1*10^1*eye(3),...
%     eye(3)*10^1*0 ,...
%     zeros(3)+eye(3)*6*10^1); %OC - case 3
% P0_ = blkdiag(zeros(3)+eye(3)*10^2*9 ,...
%     eye(3)*10^1*0 ,...
%     1*10^1*eye(3),...
%     eye(3)*10^1*0 ,...
%     zeros(3)+eye(3)*6*10^1); %OC - case 6
P0_ = blkdiag(zeros(3)+eye(3)*10^2*7 ,...
    eye(3)*10^1*0 ,...
    1*10^1*eye(3),...
    eye(3)*10^1*0 ,...
    zeros(3)+eye(3)*6*10^2); %ukf

% P0_ = blkdiag(zeros(3)+eye(3)*10^0*2 ,...
%     eye(3)*10^1*0 ,...
%     1*10^1*eye(3),...
%     eye(3)*10^1*0 ,...
%     zeros(3)+eye(3)*6*10^2); % OC


state.P = P0_;
% n = [n_w , n_a , n_wb , n_ab]
% params.Q = zeros(dim_n);    
omega_mean = mean(IMURead(:,2:4)) ;
a_mean     = mean(IMURead(:,5:7)) ;
% n = [n_w , n_wb , n_a  , n_ab] OCEKF
params.Q = blkdiag(diag(omega_mean.^2)*10^-4 ,...
    diag(bw.^2)*10^-8  ,...
    diag(a_mean.^2)*10^-4 ,...
    diag(ba.^2)*10^-8);
% params.Q = blkdiag(diag(omega_mean.^2)*10^-4 ,...
%     diag(a_mean.^2)*10^-4 ,...
%     diag(bw.^2)*10^-8  ,diag(ba.^2)*10^-8);
...
%         propagation        
    ...
    params.dt = dT;
    params_right.dt = dT;
    [state, params] = OCEKF_predict(state, params  , omega , a , g , e3);
%     disp(state.x')
...
%             params.R = (z_std+0.098898)^2*eye(pointsNum*3);  %easy    % Measurement noise matrix
            params.R = (z_std+0.099538)^2*eye(pointsNum*3)*1e2;  %medium
            params_right.R = (z_std+0.099538)^2*eye(pointsNum*3);
%             pointsB = pointsB + z_std*randn(size(pointsB));
            [state, params] = OCEKF_update(state, params,...
...
%                 pointsKeyR, path_to_save);
    ...
    Xhat = OCEKF2QUKFstate(state.x);
%     state.x(11:16) = [bw'*1;ba'*1];
end %endfor
...
filename = ['data_OCEKF_' dateTimeString '_' num2str(setpath) '.mat'];
