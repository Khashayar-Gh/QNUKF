...
load('data_newgood_repeatable_keep.mat');
...
nexttile(4);
y = vecnorm(rotv_e');
plot(Time , y , 'LineWidth',2 , 'Color','b')
% title('Small Plot 1');
xlim([Time(1) , Time(end)])
y_min = min(y);
y_max = max(y);
...
for i = 1:3
    ...
    if i == 3 % Only add xlabel for the last row
        customPlot(Time, rotv_e(:, i), 'Time [s]', ['r_{e' num2str(i) ', k} [rad.m]'], 'plot', 'r', 'linewidth', 3);
        xlabel('Time [s]', 'FontSize', axisFontSize);
    else
        customPlot(Time, rotv_e(:, i), '', ['r_{e' num2str(i) ', k} [rad.m]'], 'plot', 'r', 'linewidth', 3);
    end
...
for i = 1:3
    ax = nexttile(3 * (i - 1) + 2);
    if i == 3 % Only add xlabel for the last row
        customPlot(Time, p_e(:, i), 'Time [s]', ['p_{e' num2str(i) ', k} [m]'], 'plot', 'r', 'linewidth', 3);
        xlabel('Time [s]', 'FontSize', axisFontSize);
    else
        customPlot(Time, p_e(:, i), '', ['p_{e' num2str(i) ', k} [m]'], 'plot', 'r', 'linewidth', 3);
    end
...
for i = 1:3
    ax = nexttile(3 * (i - 1) + 3);
    if i == 3 % Only add xlabel for the last row
        customPlot(Time, v_e(:, i), 'Time [s]', ['v_{e' num2str(i) ', k} [m/s]'], 'plot', 'r', 'linewidth', 3);
        xlabel('Time [s]', 'FontSize', axisFontSize);
    else
        customPlot(Time, v_e(:, i), '', ['v_{e' num2str(i) ', k} [m/s]'], 'plot', 'r', 'linewidth', 3);
    end
