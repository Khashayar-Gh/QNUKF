...
% Xhat = [qhat ; 1.00*p+7.0*[1, 1, -2]'*1 ;...
%     0*v+0.0*randn(3,1);bw'*1;ba'*1]; %%mid
Xhat = [qhat ; 1.50*p+1.5*[1, 1, -2]' ;...
    1*v+0.0*randn(3,1);bw'*1;ba'*1]*10000; %%UPF
Xhat_log(: , 1) = Xhat;
...
P_q_indices = {[1 , 2 , 3]};
dim_n = 12;
[state, params] = initialize_ekf(dim_x, dim_z, dt, dim_n,...
...
% P0_ = blkdiag(zeros(3)+eye(3)*10^2*5 , zeros(3)+eye(3)*6*10^2 , 1*10^1*eye(3) , eye(6)*10^0*0); %New EKF data_QNUKF_20240816_144346_1.mat
% P0_ = blkdiag(zeros(3)+eye(3)*10^2*9 , zeros(3)+eye(3)*6*10^1 , 1*10^1*eye(3) , eye(6)*10^0*0); %case 3
% P0_ = blkdiag(zeros(3)+eye(3)*10^2*9 , zeros(3)+eye(3)*6*10^1 , 1*10^1*eye(3) , eye(6)*10^0*0); %case 6
P0_ = blkdiag(zeros(3)+eye(3)*10^2*7 , zeros(3)+eye(3)*6*10^2 , 1*10^1*eye(3) , eye(6)*10^0*0); %upf
...
params.Q = blkdiag(diag(omega_mean.^2)*10^-4 ,...
    diag(a_mean.^2)*10^-4 ,...
    diag(bw.^2)*10^-8  ,diag(ba.^2)*10^-8)*1e5;
...
%         propagation        
    ...
    params.dt = dT;
    params_right.dt = dT;
    [state, params] = EKF_predict(state, params  , omega , a , g , e3);
%     disp(state.x')
...
%             params.R = (z_std+0.098898)^2*eye(pointsNum*3);  %easy    % Measurement noise matrix
            params.R = (z_std+0.099538)^2*eye(pointsNum*3)*1e2*1e5;  %medium
            params_right.R = (z_std+0.099538)^2*eye(pointsNum*3);
%             pointsB = pointsB + z_std*randn(size(pointsB));
            [state, params] = EKF_update(state, params,...
...
filename = ['data_EKF_' dateTimeString '_' num2str(setpath) '.mat'];
